...
def read_abbrevs(file):
    abbrevs = {'B': [], 'N': [], 'S': []}
    for line in open(path.join(this_directory, file), encoding='utf8'):
        if not line.startswith('#'):
            abbrev, abb_type = line.strip().split('\t')[:2]
            abbrevs[abb_type].append(abbrev)
    return abbrevs


abbrevs = {
    'sr': read_abbrevs('treetagger/sr.abbrev'),
}
...
for lang in abbrevs:
    for a_type in abbrevs[lang]:
        abbrevs[lang][a_type] = list(set([e.replace('\\.', '.') for e in abbrevs[lang][a_type]]))
...
def tokenize_par(tokenizer, paragraph):
...
def tokenize(text, keepspace=False):
    ...
    for line in text:
        ...
        sentences = sentence_split(tokenize_par(tokenizer, line), 'sr')
